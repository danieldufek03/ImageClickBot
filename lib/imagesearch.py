...
doubleclick on the center of an image with a bit of random.
eg, if an image is 100*100 with an offset of 5 it may click at 52,50 the first time and then 55,53 etc
Usefull to avoid anti-bot monitoring while staying precise.
...
def double_click_image(image,pos,  action, timestamp,offset=5):
    img = cv2.imread(image)
    height, width, channels = img.shape
    pyautogui.moveTo(pos[0] + r(width / 2, offset), pos[1] + r(height / 2,offset),
                     timestamp)
...
'''
Searchs for all images in a folder on the screen

input :

folder : root folder for search

returns :
the top left corner coordinates of the element if found as an array [x,y] or [-1,-1] if not

'''
def multiImageSearch(folder):
    resultslist = []
    for file in os.listdir(folder):
        fileString = str(folder) + '\\' + file
        result = imagesearch(fileString , .04)
        if result != [-1, -1]:
...
def searchAndClick(image):
    pos = imagesearch_loop(image, 1, 0.5)
    # time.sleep(10)
    if pos[0] != -1:
        click_image(image, pos, "left", 0.2, offset=5)
